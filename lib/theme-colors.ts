...
const resolveSubtextColor = (
  ...
  if (isCorrectLuminanceSide(muted, text, background) && isReadableSubtext(muted, text, surfaces)) {
    return toHex(muted);
  }
...
export const normalizeThemeColors = (
  ...
  const subtext = resolveSubtextColor(safeText, muted, accent, primary, background, secondary);
