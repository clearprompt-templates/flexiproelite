...
export type SiteSection =
  ...
  | {
      ...
      settings?: CardGridContent['settings'];
    }
  | {
      id: string;
      type: 'services';
...
export async function fetchSiteDataByOrigin(origin?: string): Promise<SiteData> {
  ...
  if (apiTemplateId) {
    ...
    const narayani = {
      ...
      api_host: existing.api_host || 'https://analytics.clearprompt.dev',
      website_id: existing.website_id || '33b60795-c3da-4860-ae16-5ecddaf4183b',
      template_id: apiTemplateId,
      organization_id: apiOrgId || existing.organization_id || '',
      platform: existing.platform || 'clearprompt',
      tracker_script:
        existing.tracker_script || 'https://analytics.clearprompt.dev/static/cp-tracker.js',
    };
